...
static inline void serialout(unsigned short port, unsigned char val) {
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	if (finfo != 0) {
		...
		if (appsiz >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {	    
			...
			//_start_app(0x1b, task->sel + 1000 * 8, esp, task->sel + 2000 * 8, &(task->tss.esp0));
			_start_app(0x1b, 0 * 8 + 4, esp, 1 * 8 + 4, &(task->tss.esp0));	
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		if (eax == 0) {
			i = _io_in8(0x61);
			_io_out8(0x61, i & 0x0d);
		} else {
			i = 1193180000 / eax;
			_io_out8(0x43, 0xb6);
			_io_out8(0x42, i & 0xff);
			_io_out8(0x42, i >> 8);
			i = _io_in8(0x61);
			_io_out8(0x61, (i | 0x03) & 0x0f);
		}
