...
void HariMain(void)
{
	...
	putfonts8_asc((char *)binfo->vram, binfo->scrnx,  8,  8, COL8_FFFFFF, "ABC 123");
	putfonts8_asc((char *)binfo->vram, binfo->scrnx, 31, 31, COL8_000000, "Haribote OS.");
	putfonts8_asc((char *)binfo->vram, binfo->scrnx, 30, 30, COL8_FFFFFF, "Haribote OS.");

	sprintf(s, "scrnx = %d", binfo->scrnx);
	putfonts8_asc((char *)binfo->vram, binfo->scrnx, 16, 64, COL8_FFFFFF, s);
...
void putfonts8_asc(char *vram, int xsize, int x, int y, char c, unsigned char *s)
{
	...
	for (; *s != 0x00; s++) {
		putfont8(vram, xsize, x, y, c, fontdata_8x16 + *s * 16);
		x += 8;
	}
	return;
}
